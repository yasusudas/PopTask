...
**原因:** Azure のアプリ登録が **組織アカウントのみ**（シングルテナント）になっており、個人用 Microsoft アカウント（`@outlook.com` / `@hotmail.com` 等）が許可されていません。Puffy のコード側の不具合ではありません。

**対処（既存のアプリ登録を修正）:**

1. [Azure Portal](https://portal.azure.com/) → **Microsoft Entra ID** → **アプリの登録** → `Puffy` を開く
2. 左メニュー **認証**（Authentication）を開く
3. **サポートされているアカウントの種類** で次を選択:
   - **「任意の組織ディレクトリ内のアカウント (任意の Microsoft Entra ID テナント - マルチテナント) と個人用 Microsoft アカウント (Skype、Xbox など)」**
   - 英語 UI なら: *Accounts in any organizational directory and personal Microsoft accounts*
4. **保存**
5. **リダイレクト URI** に `https://puffy-dc442.firebaseapp.com/__/auth/handler`（種類: Web）があることを確認
6. [Firebase Console → Microsoft](https://console.firebase.google.com/project/puffy-dc442/authentication/providers) の **アプリケーション ID** が Azure の **アプリケーション (クライアント) ID** と一致しているか確認。シークレットを再作成した場合は Firebase 側も貼り直す

**アカウントの種類を変更できない場合:**

登録時に「この組織ディレクトリのみのアカウント」を選んでしまうと、後から個人アカウントを有効にできないことがあります。その場合は **新規登録** で作り直してください（手順 2）。新しい ID / シークレットを Firebase に登録します。

**確認用（マニフェスト）:**

アプリの **マニフェスト** で `signInAudience` が次のいずれかになっていること:

- `AzureADandPersonalMicrosoftAccount`（推奨: 職場 + 個人）
- `PersonalMicrosoftAccount`（個人のみ）

`AzureADMyOrg`（組織のみ）のままだと個人アカウントは必ず失敗します。
...
| `unauthorized_client` / not enabled for consumers | Azure のアカウント種類に「個人用 Microsoft アカウント」を含める（上記参照） |
